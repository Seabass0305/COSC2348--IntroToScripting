...
def main():		# main start
	...
	class shoe():
		...
		def fillShoe(self, tempDeckArr):
			self.__deck = tempDeckArr
	...
	class group():
		...
		def addPlayer(self, player):
			self.playerArr += player

		def displayGroup(self):
			for i in range(0, len(playerArr)):
				print(playerArr[i])
	...
	def mainGame(player1, x):
		...
		while c < 25 or c > player1.getCash(): 	# checks the bet amount is feasible
	...
	def mainMultiGame(playerArr, x):
		...
		for p in playerArr:
			...
			while c and (c < 25 or c > p.getCash()) and p.canPlay: 	# checks the bet amount is feasible
